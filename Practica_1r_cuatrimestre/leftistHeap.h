...
public:
	...
	void deleteMin(std::pair<float, int> &minElem); // Removes the smallest element in the heap and returns it, used instead getMin() followed by deleteMin()
	void insert(std::pair<int, int> elem); // Inserts the given element into the heap
	void decreaseKey(int oldKey, float newKey); // Modifies the value of the given oldKey into the newKey
